...
function(jobName, agentEnv={}, stepEnvFile='', patchFunc=identity) patchFunc({
  ...
  local env = {
    ...
    BUILDKITE_PLUGIN_K8S_GIT_CREDENTIALS_SECRET_KEY: 'git-credentials',
    BUILDKITE_PLUGIN_K8S_GIT_CREDENTIALS_SECRET_NAME: '',
    BUILDKITE_PLUGIN_K8S_GIT_SSH_SECRET_KEY: 'ssh-key',
    BUILDKITE_PLUGIN_K8S_GIT_SSH_SECRET_NAME: '',
    BUILDKITE_PLUGIN_K8S_AGENT_TOKEN_SECRET_NAME: '',
    BUILDKITE_PLUGIN_K8S_AGENT_TOKEN_SECRET_KEY: 'buildkite-agent-token',
    BUILDKITE_PLUGIN_K8S_INIT_IMAGE: 'muhlba91/buildkite-agent-k8s:latest',
    ...
    [
      {
        ...
        valueFrom: {
          secretKeyRef: {
            ...
            name: if env.BUILDKITE_PLUGIN_K8S_DEFAULT_SECRET_NAME != ''
            then
              env.BUILDKITE_PLUGIN_K8S_DEFAULT_SECRET_NAME
            else
              env.BUILDKITE_PLUGIN_K8S_AGENT_TOKEN_SECRET_NAME,
          },
        },
      },
    ] + [
  ...
  local buildVolume =
    ...
    then
      {
        persistentVolumeClaim: {
          claimName: env.BUILDKITE_PLUGIN_K8S_BUILD_PATH_PVC,
        },
      }
    ...
    then
      {
        hostPath: {
          path: env.BUILDKITE_PLUGIN_K8S_BUILD_PATH_HOST_PATH,
  ...
  local gitMirrorsVolume =
    ...
    then
      {
        hostPath: {
          path: env.BUILDKITE_PLUGIN_K8S_GIT_MIRRORS_HOST_PATH,
          type: 'DirectoryOrCreate',
        },
      }
    else
  ...
  local defaultSecretsMounts = {
    mount:
      if env.BUILDKITE_PLUGIN_K8S_DEFAULT_SECRET_NAME == ''
      then
        []
      else
        [
        ...
        [
          {
            ...
            secret: {
              ...
              items: [
                {
                  key: env.BUILDKITE_PLUGIN_K8S_GIT_CREDENTIALS_SECRET_KEY,
        ...
        [
          {
            ...
            subPath: env.BUILDKITE_PLUGIN_K8S_GIT_CREDENTIALS_SECRET_KEY,
          },
        ],
    volume:
      if env.BUILDKITE_PLUGIN_K8S_GIT_CREDENTIALS_SECRET_NAME == ''
        ...
        [
          {
            ...
            secret: {
              defaultMode: 256,
              secretName: env.BUILDKITE_PLUGIN_K8S_GIT_CREDENTIALS_SECRET_NAME,
              items: [
                {
                  key: env.BUILDKITE_PLUGIN_K8S_GIT_CREDENTIALS_SECRET_KEY,
                  path: 'git-credentials',
        ...
        [
          {
            ...
            name: 'git-ssh-key',
            subPath: env.BUILDKITE_PLUGIN_K8S_GIT_SSH_SECRET_KEY,
          },
        ],
    volume:
      if env.BUILDKITE_PLUGIN_K8S_GIT_SSH_SECRET_NAME == ''
        ...
        [
          {
            ...
            secret: {
              defaultMode: 256,
              secretName: env.BUILDKITE_PLUGIN_K8S_GIT_SSH_SECRET_NAME,
              items: [
                {
                  key: env.BUILDKITE_PLUGIN_K8S_GIT_SSH_SECRET_KEY,
                  path: 'ssh-key',
    ...
    then
    ...
    else
      {
        command: [
          env[f]
          for f in std.sort(std.objectFields(env), numberSuffix)
          if std.startsWith(f, 'BUILDKITE_PLUGIN_K8S_ENTRYPOINT_')
        ],
        args: [
          env[f]
          for f in std.sort(std.objectFields(env), numberSuffix)
          if std.startsWith(f, 'BUILDKITE_PLUGIN_K8S_COMMAND_')
        ],
      },

  local initContainers =
    if env.BUILDKITE_PLUGIN_K8S_INIT_IMAGE == ''
    then
      []
    else
      [
        ...
        containers: [
          {
            ...
            resources: {
              requests:
                (
                  if env.BUILDKITE_PLUGIN_K8S_RESOURCES_REQUEST_CPU != ''
                  then
                    {
                ...
                +
                (
                  if env.BUILDKITE_PLUGIN_K8S_RESOURCES_REQUEST_MEMORY != ''
                  then
                    {
              ...
              limits:
                (
                  if env.BUILDKITE_PLUGIN_K8S_RESOURCES_LIMIT_CPU != ''
                  then
                    {
                ...
                +
                (
                  if env.BUILDKITE_PLUGIN_K8S_RESOURCES_LIMIT_MEMORY != ''
                  then
                    {
            ...
            volumeMounts: [
                            {
                              mountPath: env.BUILDKITE_PLUGIN_K8S_WORKDIR,
                              name: 'build',
